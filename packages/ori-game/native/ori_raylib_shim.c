...
static const char *cstr(int ptr) {
    if (ptr == 0) return "";
...
int ori_rl_LoadTexture(int fileName) {
    ...
    if (!id) return 0;
    g_tex[id] = LoadTexture(cstr(fileName));
    return id;
}
...
int ori_rl_LoadModel(int fileName) {
    ...
    if (!id) return 0;
    g_model[id] = LoadModel(cstr(fileName));
    return id;
}
...
int ori_rl_LoadShader(int vsPath, int fsPath) {
    int id = alloc_shader();
    if (!id) return 0;
...
int ori_rl_GetShaderLocation(int shader, int name) {
...
void ori_rl_TakeScreenshot(int fileName) {
...
#include <stdio.h>
#include <stdlib.h>
void ori_rl_OpenURL(int url) { OpenURL((const char *)(intptr_t)url); }
void ori_rl_OpenPath(int path) {
    const char *p = (const char *)(intptr_t)path;
    if (!p || !p[0]) return;
    char resolved[PATH_MAX];
    char url[PATH_MAX + 16];
    if (realpath(p, resolved) != NULL) snprintf(url, sizeof(url), "file://%s", resolved);
    else snprintf(url, sizeof(url), "file://%s", p);
    OpenURL(url);
}
